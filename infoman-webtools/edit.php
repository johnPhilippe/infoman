...
$fname = $lname = $username = $city = $email = $userType = $phone_Num = "";
$fname_err = $lname_err = $username_err = $city_err = $email_err = $userType_err = $phone_Num_err = "";
...
if(isset($_POST["id"]) && !empty($_POST["id"])){
   ...
   // Validate name
    ...
    if(empty($input_userType)){
        $userType_err = "Please enter user type.";     
    } else{
    ...
    if(empty($fname_err) && empty($lname_err) && empty($username_err) && empty($city_err) && empty($email_err) && empty($userType_err) && empty($phone_Num_err)){
        // Prepare an update statement
        $sql = "UPDATE tbl_users SET first_Name=?, last_Name=?, username=?, city=?, email=?, userType=?, phone_Num=? WHERE id=$id";
        
        if($stmt = mysqli_prepare($con, $sql)){
            // Bind variables to the prepared statement as parameters
             mysqli_stmt_bind_param($stmt, "sssssss", $param_fname, $param_lname, $param_username, $param_city, $param_email, $param_userType, $param_phone_num);
             echo "update hello";
             // Set parameters
            $param_fname = $fname;
            $param_lname = $lname;
